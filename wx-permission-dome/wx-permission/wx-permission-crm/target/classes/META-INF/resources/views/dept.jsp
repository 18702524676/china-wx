...
<div id="dialog-user-form" style="display: none;">
    <form id="userForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr id="userPasswordTr">
                <td><label for="userTelephone">密码</label></td>
...
<script type="application/javascript">
    $(function() {
        ...
        function loadDeptTree() {
            $.ajax({
                url: "/permission/sys/dept/getDeptTree.json",
        ...
        function recursiveRenderDept(deptList) {
            if(deptList && deptList.length > 0) {
                $(deptList).each(function (i, dept) {
                     ...
                     if (dept!= null && dept.treeVOList != null && dept.treeVOList.length > 0) {
                         var rendered = Mustache.render(deptListTemplate, {deptList: dept.treeVOList});
                         $("#dept_" + dept.id).append(rendered);
                         recursiveRenderDept(dept.treeVOList);
                     }
                })
            }
        }
                    ...
                    buttons : {
                        "更新": function(e) {
                            ...
                            updateDept(false, function (data) {
                                showMessage("更新部门", data.message, true);
        ...
        function loadUserList(deptId) {
            ...
            var url = "/permission/sys/user/pageUser.json?deptId=" + deptId;
        ...
        function renderUserListAndPage(result, url) {
            if (result.status == 200) {
               ...
               /* var pageNo = $("#userPage .pageNo").val() || 1;*/
                ...
                var total = result.data == null ? 0 : result.data.total;
                var currentSize =  total > 0 ? result.data.data.length : 0;
                renderPage(url,total, pageNo, pageSize, currentSize, "userPage", renderUserListAndPage);
            } else {
                ...
                buttons : {
                    "添加": function(e) {
                        ...
                        updateUser(true, function (data) {
                            showMessage("新增用户", data.message, true);
                            $("#dialog-user-form").dialog("close");
                            loadUserList(lastClickDeptId);
                        }, function (data) {
                            showMessage("新增用户", data.message, false);
                        })
                    },
                    "取消": function () {
                        $("#dialog-user-form").dialog("close");
                ...
                $("#dialog-user-form").dialog({
                    ...
                    open: function(event, ui) {
                        ...
                        if (targetUser) {
                           ...
                           /* $("#userPassword").val(targetUser.password);*/
                            $("#userPasswordTr").hide();
                    ...
                    buttons : {
                        "更新": function(e) {
                            ...
                            updateUser(false, function (data) {
                                showMessage("更新用户", data.message, true);
                                $("#dialog-user-form").dialog("close");
                                loadUserList(lastClickDeptId);
                            }, function (data) {
                                showMessage("更新用户", data.message, false);
                            })
                        },
                        "取消": function () {
                            $("#dialog-user-form").dialog("close");
                ...
                buttons : {
                    "添加": function(e) {
                        ...
                        updateDept(true, function (data) {
                            showMessage("新增部门", data.message, true);
                            $("#dialog-dept-form").dialog("close");
                        }, function (data) {
        ...
        function recursiveRenderDeptSelect(deptList, level) {
            ...
            if (deptList && deptList.length > 0) {
                $(deptList).each(function (i, dept) {
                    ...
                    if (dept!= null && dept.treeVOList != null && dept.treeVOList && dept.treeVOList.length > 0) {
                        recursiveRenderDeptSelect(dept.treeVOList, level + 1);
                    }
                });
            }
        }
        ...
        function updateUser(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "/permission/sys/user/saveUser.json" : "/permission/sys/user/updateUser.json",
        ...
        function updateDept(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "/permission/sys/dept/saveDept.json" : "/permission/sys/dept/updateDept.json",
