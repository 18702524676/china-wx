...
<div id="dialog-acl-form" style="display: none;">
    <form id="aclForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr>
                <td><label for="aclUrl">URL或权限标识</label></td>
...
<script type="text/javascript">
    $(function() {
        ...
        function loadAclModuleTree() {
            $.ajax({
                url: "/permission/sys/aclModule/getAclModuleTree.json",
                        ...
                        var rendered = Mustache.render(aclModuleListTemplate, {
                            ...
                            "showDownAngle": function () {
                                return function (text, render) {
                                    return (this.treeVOList && this.treeVOList.length > 0) ? ">>" : "hidden";
                                }
                            },
                ...
                buttons : {
                    "添加": function(e) {
                        ...
                        updateAclModule(true, function (data) {
                            showMessage("新增权限模块", data.message, true);
        ...
        function updateAclModule(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "/permission/sys/aclModule/saveSysAclModule.json" : "/permission/sys/aclModule/updateSysAclModule.json",
        ...
        function updateAcl(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "/permission/sys/acl/saveSysAcl.json" : "/permission/sys/acl/updateSysAcl.json",
            ...
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, aclModule) {
                    ...
                    if (aclModule.treeVOList && aclModule.treeVOList.length > 0) {
                        recursiveRenderAclModuleSelect(aclModule.treeVOList, level + 1);
                    }
                });
            }
        }
        ...
        function recursiveRenderAclModule(aclModuleList) {
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, aclModule) {
                    ...
                    if (aclModule.treeVOList && aclModule.treeVOList.length > 0) {
                        var rendered = Mustache.render(aclModuleListTemplate, {
                            aclModuleList: aclModule.treeVOList,
                            "showDownAngle": function () {
                                return function (text, render) {
                                    return (this.treeVOList && this.treeVOList.length > 0) ? "" : "hidden";
                                }
                        ...
                        recursiveRenderAclModule(aclModule.treeVOList);
                    }
                })
            }
        }
        ...
        function bindAclModuleClick() {
            ...
            $(".aclModule-delete").click(function (e) {
                ...
                if (confirm("确定要删除权限模块[" + aclModuleName + "]吗?")) {
                    $.ajax({
                        url: "/permission/sys/aclModule/delete.json",
                        data: {
                            id: aclModuleId
                        },
                        type: 'POST',
                    ...
                    buttons : {
                        "更新": function(e) {
                            ...
                            updateAclModule(false, function (data) {
                                showMessage("编辑权限模块", data.message, true);
                                $("#dialog-aclModule-form").dialog("close");
                            }, function (data) {
        ...
        function loadAclList(aclModuleId) {
            ...
            var url = "/permission/sys/acl/pageSysAcl.json?aclModuleId=" + aclModuleId;
        ...
        function renderAclListAndPage(result, url) {
            if(result.status == 200) {
                ...
                renderPage(url,total, pageNo, pageSize, currentSize, "aclPage", renderAclListAndPage);
            } else {
        ...
        function bindAclClick() {
            $(".acl-role").click(function (e) {
                ...
                $.ajax({
                    url: "/permission/sys/acl/acls.json",
                    data: {
                        aclId: aclId
                    },
                    success: function(result) {
                    ...
                    buttons : {
                        "更新": function(e) {
                            ...
                            updateAcl(false, function (data) {
                                showMessage("编辑权限", data.message, true);
                ...
                buttons : {
                    "添加": function(e) {
                        ...
                        updateAcl(true, function (data) {
                            showMessage("新增权限", data.message, true);
                            $("#dialog-acl-form").dialog("close");
                        }, function (data) {
